...
- (CGFloat)createBBIWithBBIImageName:(NSString *)bbiImageName andX:(CGFloat)x andTag:(int)tag andClass:(id)classObject andSEL:(SEL)sel
{
    ...
    btn.frame=CGRectMake(0*screenWidth/320, 5*screenWidth/320, 50*screenWidth/320, 30*screenWidth/320);
    btn.imageEdgeInsets = UIEdgeInsetsMake(0*screenWidth/320,10*screenWidth/320,0*screenWidth/320,20*screenWidth/320);
   // btn.contentMode=UIViewContentModeScaleAspectFit;
    [btn setImage:image forState:UIControlStateNormal];
    //[btn setBackgroundImage:image forState:UIControlStateNormal];
    btn.tag = tag;
    [btn addTarget:classObject action:sel forControlEvents:UIControlEventTouchUpInside];
    if([bbiImageName isEqualToString:@"fanhui.png"])
    {
