...
{
    UIButton *_altBtn;//选择结算银行按钮
    
   // UITextField *_name;
    
    //UITextField *_number;
    
    UITextField *_nameId;//身份证号

    //UITextView *_fenhang;
    
   // UILabel *_lab;
    
    UITextField *_field;//银行卡号
    
    UIButton *_upBtn;//上传照片的按钮
    
   // UIButton *_downBtn;
    
    AVCaptureSession * _AVSession;//调用闪光灯的时候创建的类
    
    UIActionSheet *_as;//银行选择器
    
    NSString *_str;//结算银行
    
    NSDictionary *dic;//绑定银行卡动作后，接口返回000代码的数据转成的字典
    ...
    NSDictionary *_dic1;//绑定银行卡动作后，接口返回008代码的数据转成的字典
    ...
    //scrollerView布局
    UIScrollView *_scr;//SCROLL滚动view

    
    MBProgressHUD* progress_;
    //姓名
    UITextField *fielab;//户主名
...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"绑定银行卡" andClass:self andSEL:@selector(bbaClick:)];
...
-(void)btton
{
    ...
    {
        [self toastResult:@"网络连接异常,请检查后重试"];
    }
    else
    {
        User * user = [User currentUser];
        ...
        //NSString *paizhaoString=[NSString stringWithFormat:@"%@/ss/uploadAttach.do",url1];
        [manager POST:@"http://120.26.91.233/mobile/ss/uploadAttach.do" parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData)
         {
...
- (void)btndown
{
    ...
    if ([[_dic1 objectForKey:@"respCode"] isEqualToString:@"000"]) {
        ...
        if ([[dic objectForKey:@"respCode"] isEqualToString:@"000"])
        {
            if([user.isFirstTrans isEqualToString:@"N"])
            {
                UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"请支付领取199元话费" delegate:self cancelButtonTitle:nil otherButtonTitles:@"支付", nil];
                [alertView show];
                [progress_ hide:YES];
                alertView.tag=4;
            }
