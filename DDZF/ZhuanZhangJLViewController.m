...
-(void)makeNav
{
    ...
    mnb.frame = CGRectMake(0, 20*screenWidth/320.0, self.view.frame.size.width, 44*screenWidth/320.0);
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"转账记录" andClass:self andSEL:@selector(bacClick:)];
...
-(void)makeUI
{
    _tableview= [[UITableView alloc] initWithFrame:CGRectMake(0, 114.6*screenWidth/320.0, screenWidth, self.view.frame.size.height-114.6*screenWidth/320.0) style:UITableViewStylePlain];//实例化，两种类型plain、group
    //两个代理
...
//
    ...
    //全部收款和成功交易之间的竖线
    UIView *view=[[UIView alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2-1, 64.6*screenWidth/320.0, 1, 50*screenWidth/320.0)];
    view.backgroundColor=[UIColor grayColor];
    [self.view addSubview:view];
    
    //全部收款和成功交易底部的横线
    UIView *view1=[[UIView alloc] initWithFrame:CGRectMake(0,114.6*screenWidth/320.0, self.view.frame.size.width, 1)];
...
//    cell.timeString.text=_dataArray[indexPath.row][@"createDate"];
//    
...
-(void)makebutton
{
    ...
    _button1.frame=CGRectMake(0, 64.6*screenWidth/320.0, screenWidth/2, 50*screenWidth/320.0);
    ...
    _button2.frame=CGRectMake(screenWidth/2, 64.6*screenWidth/320.0, screenWidth/2, 50*screenWidth/320.0);
