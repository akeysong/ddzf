...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"添加银行卡" andClass:self andSEL:@selector(bacClick:)];
...
-(void)makeUI
{
    ...
    for(int i=0;i<arr.count;i++)
    {
        ...
        backView=[[UIView alloc] initWithFrame:CGRectMake(10*screenWidth/320.0, (90+i*60)*screenWidth/320.0,self.view.frame.size.width- 20*screenWidth/320.0, 50*screenWidth/320.0)];
    ...
    btn.backgroundColor=[UIColor colorWithRed:0.19 green:0.8 blue:0.0 alpha:1];
...
-(void)choseAlert:(UIButton*)btn
{
    ...
    _al = [[UIActionSheet alloc]initWithTitle:@"请选择银行" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"中国工商银行",@"中国农业银行",@"中国建设银行",@"中国交通银行",@"招商银行",@"中信银行",@"光大银行",@"北京银行",@"浦发银行",@"民生银行", nil];
...

{
    ...
    switch (buttonIndex) {
        ...
        case 8:
            //_imagStr=@"ios320x568_16.png";
            _cardStr=@"spdb";
            [_choice setTitle:@"浦发银行" forState:UIControlStateNormal];
            break;

        case 9:
            //_imagStr=@"ios320x568_16.png";
            _cardStr=@"cmbc";
            [_choice setTitle:@"民生银行" forState:UIControlStateNormal];
            break;
