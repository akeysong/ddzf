...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"更改登录密码" andClass:self andSEL:@selector(bacClick:)];
...
-(void)button:(UIButton*)btn
{
    ...
    NSLog(@"密码为%@",user.yuanText);
        ...
        {
        [self toastResult:@"密码由6位或6位以上数字或字母组成"];
        return;
     ...
     if(_log.text.length <6){
        [self toastResult:@"密码由6位或6位以上数字或字母组成"];
        return;
         
    }
    if (_ok.text == nil ||[@"" isEqual:_ok.text]) {
        [self toastResult:@"请输入确认密码"];
        return;
    }
    if(_ok.text.length <6){
        [self toastResult:@"密码由6位或6位以上数字或字母组成"];
        return;
