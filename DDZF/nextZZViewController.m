...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"转账第二步" andClass:self andSEL:@selector(bacClick:)];
...
-(void)makeUI
{
    ...
    if(num<=0)
    {
        numTextField.placeholder=@"可转帐金额为0.00元";
    }
    else
    {
         numTextField.placeholder=[NSString stringWithFormat:@"可转账金额为%.2f元",num];
    }
...
-(void)nextBtn
{
    ...
    if(_textView.text.length<=0||_textView.text.length>=20)
    {
        [self toastResult:@"转账说明在1~20个字符内" ];

    }
    NSLog(@"输出转账信息为%f,%f,%f",[numTextField.text doubleValue],[user.KYyue doubleValue],[user.totAmtT1 doubleValue]);
    if([numTextField.text doubleValue]>([user.KYyue doubleValue]-[user.totAmtT1 doubleValue]))
    {
        UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"余额不足,请充值" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alertView show];
        alertView.tag=3;
    }
    else
