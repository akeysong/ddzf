...
-(void)makeNav
{
    ...
    [mnb createMyNavigationBarWithBgImageName:nil andLeftBBIIamgeNames:[NSArray arrayWithObject:@"fanhui.png"] andRightBBIImages:nil andTitle:@"充值记录" andClass:self andSEL:@selector(bacClick:)];
...
-(void)makeUI
{
    ...
    _table= [[UITableView alloc] initWithFrame:CGRectMake(0, 114.6*screenWidth/320, self.view.frame.size.width, self.view.frame.size.height-114.6*screenWidth/320) style:UITableViewStylePlain];//实例化，两种类型plain、group
    //两个代理
...
-(void)makebutton
{
    ...
    _button1.frame=CGRectMake(0, 64.6*screenWidth/320, screenWidth/2, 50*screenWidth/320);
    //   [_button1 setTitle:@"" forState:UIControlStateNormal];
    [_button1 makeUI:@"全部充值" :YES];
...
-(void)LoadData
{
    ...
    if(_dataArr.count<1)
    {
        UIAlertView *alat = [[UIAlertView alloc] initWithTitle:@"提示" message:@"近期内你还没有此记录" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    nextChongZhiJLViewController *next=[[nextChongZhiJLViewController alloc] init];
